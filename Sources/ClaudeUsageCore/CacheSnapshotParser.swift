...
public struct CacheSnapshotParser: Sendable {
    ...
    public func parse(text: String, capturedAt: Date? = nil) -> CacheSnapshot? {
        if text.contains("/v1/code/routines/run-budget"),
           let payload = firstJSONObject(in: text),
        ...
        if text.contains("/usage"),
           let payload = firstJSONObject(in: text),
           let used = firstDouble(in: payload, keys: ["used", "amount_used", "current_spend", "spent"]),
           let limit = firstDouble(in: payload, keys: ["limit", "spend_limit", "monthly_limit", "amount_limit"]) {
